...
class DomainCredibilityScorer:
    ...
    def resolve_conflicts(self, property_name: str, claims: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Uses epsilon tolerance (1e-4) matching to group numerical values safely.
        """
        if not claims:
            return []
        ...
        # Group by numerical value with epsilon tolerance and unit
        groups: List[Dict[str, Any]] = []
        for claim in claims:
            val = float(claim["value"])
            unit_lower = claim.get("unit", "").lower()
            matched_group = None
            for g in groups:
                if g["unit"] == unit_lower and abs(g["val"] - val) < 1e-4:
                    matched_group = g
                    break
        ...
        group_scores = []
        for g in groups:
            s_d = sum(c["score_contrib"] for c in g["claims"])
            group_scores.append((g, s_d))

        if not group_scores:
            return []

        max_score = max(s_d for _, s_d in group_scores)
        threshold = 0.35 * max_score

        # Keep groups that meet the threshold
        valid_claims = []
        for g, s_d in group_scores:
            if s_d >= threshold:
                best_claim = g["claims"][0].copy()
                best_claim["S_d"] = s_d
...
class SmartSourceFilter:
    ...
    def __init__(self):
        ...
        self.spam_domain_pattern = re.compile(
            r"(?:" + "|".join(map(re.escape, self.spam_domain_keywords)) + r")",
            re.IGNORECASE
        )
        self.spam_content_pattern = re.compile(
            r"(?:" + "|".join(map(re.escape, self.spam_content_keywords)) + r")",
            re.IGNORECASE
        )
    ...
    def is_spam(self, url: str, title: str = "", snippet: str = "") -> bool:
        ...
        # Check spam domain keywords via pre-compiled regex
        if self.spam_domain_pattern.search(domain):
            return True
            
        # Check spam content keywords in title or snippet via pre-compiled regex
        text = f"{title} {snippet}".lower()
        if self.spam_content_pattern.search(text):
            return True
